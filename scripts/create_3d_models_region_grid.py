...
def add_mask_mesh(
    ...
    Args:
        ...
        rasterize: Whether to rasterize the mesh in the PDF output.
...
def draw_grid_cell(
    ...
    Args:
        ...
        rasterize_pdf: Whether to rasterize the mesh in the PDF output.
    """
...
def save_models_region_grid(
    ...
    """Saves a grid comparing ground truth and model predictions by region.

    Args:
        case_id: Case identifier.
        gt_label_map: Ground-truth label map.
        predictions: Dictionary with one prediction per model.
        output_dir: Directory where the PDF figure is saved.
        step_size: Marching-cubes step size.
        smooth_sigma: Gaussian smoothing sigma.
        elev: Camera elevation.
        azim: Camera azimuth.
        ...
        rasterize_pdf: Whether to rasterize meshes in the PDF output.
    """
    ...
    pdf_path = output_dir / f"{case_id}_models_region_grid.pdf"
    ...
    print(f"[OK] PDF figure saved at: {pdf_path}")

    if not rasterize_pdf:
        print(
            "[INFO] Vector PDF saved. It may be larger, "
            "but it should not look pixelated."
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Rasterize meshes inside the PDF. "
            "This reduces file size but may make the figure look pixelated."
        ),
    )
...
def main() -> None:
    ...
    create_models_grid(
        ...
        rasterize_pdf=args.rasterize_pdf,
    )
